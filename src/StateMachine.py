...
class StateMachine:
    ...
    def get_main_machine(model, states):
        if StateMachine._main_machine is None:
            StateMachine._main_machine = Machine(model=model, states=states, initial='initialised')
    ...
    def get_bt_server_machine(model, states):
        if StateMachine._bt_server_machine is None:
            StateMachine._bt_server_machine = Machine(model=model, states=states, initial='initialised')
    ...
    def get_stepperh_machine(model, states):
        if StateMachine._stepperh_machine is None:
            StateMachine._stepperh_machine = Machine(model=model, states=states, initial='initialised')
    ...
    def get_stepperv_machine(model, states):
        if StateMachine._stepperv_machine is None:
            StateMachine._stepperv_machine = Machine(model=model, states=states, initial='initialised')
    ...
    def get_magnet_machine(model, states):
        if StateMachine._magnet_machine is None:
            StateMachine._magnet_machine = Machine(model=model, states=states, initial='initialised')
    ...
    def get_camera_machine(model, states):
        if StateMachine._camera_machine is None:
            StateMachine._camera_machine = Machine(model=model, states=states, initial='initialised')
